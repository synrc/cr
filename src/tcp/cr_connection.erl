...
%    io:format("SERVER: SEND ~p~n", [Message]),
...
transfer(_Data,  State) ->
    io:format("SERVER: unknown Data during transfer: ~p\n", [_Data]),
    {stop, normal, State}.
...
init([Name,Mod,Socket,Nodes]) ->
    io:format("PROTOCOL: starting ~p listener: ~p~n",[self(),{Name,Mod}]),
...
handle_info(_Info, StateName, State) ->
    io:format("SERVER: Info ~p~n",[_Info]),
